  ...
  <div class="login-wrap">
    <el-form label-position="left" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="0px" class="demo-ruleForm login-container">
      ...
      <el-form-item style="width:100%; text-align: center; margin-top: 10px;">
        <el-button type="text" @click="goToRegister" style="color: #409EFF;">
          还没有账号？立即注册
        </el-button>
      </el-form-item>
    </el-form>
...
import { getAndClearRedirectPath, getDefaultRedirectPath } from '../utils/auth'
export default {
  name: 'login',
  data() {
    return {
  ...
  methods: {
    ...
    redirectByRole(userData) {
      // 检查是否有重定向路径（从购物车等页面跳转过来）
      const redirectPath = this.$route.query.redirect || getAndClearRedirectPath()
      ...
      // 根据用户角色进行默认跳转
      const userRole = userData.role || userData.roleName || 'user'
      
      if (userRole === 'admin' || userRole === 'ADMIN' || userRole === '管理员') {
        // 管理员跳转到订单管理页面
        this.$router.push({ path: '/admin/Ticket' })
